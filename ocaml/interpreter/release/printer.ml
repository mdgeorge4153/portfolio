...
let format_stream fmt_element f s =
  Format.fprintf f "@[[";

  let rec helper n (Stream (h,tl)) =
    if n > 0
    then begin
      Format.fprintf f "%a, " fmt_element h;
      helper (n-1) (tl ())
    end
    else
      Format.fprintf f "...]@]"
...
let format_list format_elt f l =
  Format.fprintf f "@[[";
  List.iter (Format.fprintf f "%a;" format_elt) l;
  Format.fprintf f "]@]";
  ()
...
let print_stream_int      = make_printer format_stream_int
let print_stream_float    = make_printer format_stream_float
let print_stream_int_list = make_printer format_stream_int_list
...
let string_of_stream_int      = make_string_of format_stream_int
let string_of_stream_float    = make_string_of format_stream_float
let string_of_stream_int_list = make_string_of format_stream_int_list
